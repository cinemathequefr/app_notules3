...
module.exports = async function (db, cycleConfig) {
  ...
  // TEST : rapproche les séances associées, effectue une numérotation commune des items, leur donne la même : idSeance, date/heure, salle.
  seancesMerged = _(seancesMerged)
    .thru(data => {
      let ignore = [];
      return _(data).reduce((acc, i) => {
        if (_.isUndefined(i.idSeanceAssoc)) {
