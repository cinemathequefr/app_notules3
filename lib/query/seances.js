...
async function getSeancesFromCats(db, idCats, idProg) {
  let seancesFilms = await execQuery.single(db, queries.seancesFromCats, [idCats, idProg]);
  let seancesConfs = await execQuery.single(db, queries.seancesConfsFromCats, [idCats, idProg]);


  // NON, les données ne sont pas dans la forme voulue à ce stade. Il faut le faire au moment du passage à la structure hiérarchisée, un peu plus loin.
  // // Ajoute dans chaque séance de type conférence un item de séance (idEvenement) servant de placeholder.
  // seancesConfs = _(seancesConfs)
  //   .map(d =>
  //     _(d).assign({
  //       items: [{
  //         idConf: d.idEvenement
  //       }]
  //     }).value()
  //   )
  //   .value();
